...
app.put('/api/transaksi/:id', (req, res) => {
  ...
  const index = dbTransaksi.findIndex(t => t.id === id);
  if (index !== -1) {
    dbTransaksi[index] = { id, ...req.body };
    res.json(dbTransaksi[index]);
  } else {
...
app.get('/api/hitung', (req, res) => res.json(hitungSaldo()));
app.get('/api/ringkasan', (req, res) => {
  const saldoList = hitungSaldo();
  const jumlah = saldoList.length;
  if (jumlah === 0) return res.json({ totalHarta: 0, totalUtang: 0, totalModal: 0, seimbang: true, jumlahTransaksi: 0, detail: { kas: 0, perlengkapan: 0, peralatan: 0 } });
  const last = saldoList[jumlah - 1].spiegel;
  const totalHarta = last.kas + last.perlengkapan + last.peralatan;
  res.json({
    totalHarta, totalUtang: last.utangUsaha, totalModal: last.modal,
    seimbang: totalHarta === last.utangUsaha + last.modal,
